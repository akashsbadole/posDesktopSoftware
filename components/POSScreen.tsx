      ...
      >
        <div className="p-3 border-b border-border bg-[#0D0D0F]/50">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <div className="flex bg-[#141418] rounded-md p-0.5 border border-[#1E1E26]" role="group" aria-label="Price Tier">
                <button
            ...
            <div className="flex gap-1.5 items-center">
              <button onClick={handleNoSale} className="p-1.5 rounded-lg text-[#2ECC71] bg-green-500/5 border border-green-500/10 hover:bg-green-500/10" title="Open Drawer"><Banknote size={14} /></button>
              <button onClick={handleClearCart} className="p-1.5 rounded-lg text-[#E74C3C] bg-red-500/5 border border-red-500/10 hover:bg-red-500/10" title="Clear Cart"><Trash2 size={14} /></button>
              <button onClick={() => setShowHeldOrders(true)} className="p-1.5 rounded-lg text-[#F5C842] bg-yellow-500/5 border border-yellow-500/10 hover:bg-yellow-500/10 relative">
                <Clock size={14} />
                {heldOrders.length > 0 && <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-red-500 text-white text-[8px] flex items-center justify-center border-2 border-[#0D0D0F]">{heldOrders.length}</span>}
              </button>
            </div>
          </div>
          ...
          <div className="flex gap-1" role="group" aria-label="Order type">
            {[
              ...
              { id: "takeaway", label: labels.takeaway.split("/")[1] || "Takeaway" },
        ...
        <div className="px-3 pt-2">
          ...
          <div className="grid grid-cols-2 gap-1 border border-[#1E1E26] rounded-sm p-1.5 bg-[#141418]/30 mb-1.5">
            <div className="flex flex-col">
              <span className="text-[8px] uppercase font-bold text-[#4A4A5A] leading-none mb-1">Table</span>
              <button
              ...
              >
                ...
                <span className="text-[8px] text-[#F5C842] uppercase ml-1">(Edit)</span>
              </button>
            </div>
            <div className="flex flex-col border-l border-[#1E1E26] pl-1.5">
              <span className="text-[8px] uppercase font-bold text-[#4A4A5A] leading-none mb-1">Customer</span>
              <button
                onClick={() => setIsEditingCustomer(!isEditingCustomer)}
                data-testid="edit-customer-btn"
                className="text-[10px] font-bold text-white flex items-center justify-between hover:text-[#F5C842] transition-colors"
              >
                <span className="truncate">{customerInfo?.name || activeCustomer?.name || "Walk-in"}</span>
                <span className="text-[8px] text-[#F5C842] uppercase ml-1">(Edit)</span>
              </button>
            </div>
          </div>
          ...
          {isEditingCustomer && (
            <div className="space-y-2 mb-3 p-2 bg-[#141418] rounded-lg border border-[#1E1E26] slide-in">
              <div className="relative">
                <User size={12} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#4A4A5A]" />
                <input
              ...
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) => {
                    setCustomerInfo({ ...customerInfo, phone: e.target.value } as any);
              ...
              {customerAddresses.length > 0 && (
                <div className="flex gap-1 overflow-x-auto py-1">
                  {customerAddresses.map((addr) => (
                    <button
                      key={addr.id}
                      onClick={() => setCustomerInfo({ ...customerInfo, address: addr.address, phone: addr.phone, name: customerInfo?.name || "" } as any)}
        ...
        >
          ...
          {cart.map((item) => {
            ...
            return (
              <div key={item.cartItemId} className="flex items-center gap-1.5 py-1 border-b border-white/5 slide-in" role="listitem">
                <div className="flex items-center bg-[#1E1E26] rounded-sm overflow-hidden shrink-0 h-6">
                  <button onClick={() => updateQuantity(item.cartItemId, item.quantity - 1)} className="px-1 h-full hover:bg-[#F5C842] hover:text-[#0D0D0F] transition-colors" aria-label="Decrease"><Minus size={8} /></button>
                  <span className="w-5 text-center text-[9px] font-bold leading-none">{item.quantity}x</span>
                  <button onClick={() => updateQuantity(item.cartItemId, item.quantity + 1)} className="px-1 h-full hover:bg-[#F5C842] hover:text-[#0D0D0F] transition-colors" aria-label="Increase"><Plus size={8} /></button>
                </div>
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  >
                    {curr}{currentPrice.toFixed(2)}
                  </button>
                  <button onClick={() => removeItem(item.cartItemId)} className="text-[#4A4A5A] hover:text-[#E74C3C]" aria-label="Remove"><X size={10} /></button>
                </div>
              </div>
            );
          })}
          ...
          <div className="p-1.5 border-t border-border bg-[#0D0D0F]/30 space-y-1.5">
            <div className="flex gap-1 items-center">
              <div className="flex-1 flex items-center bg-[#141418] rounded-sm border border-[#1E1E26] px-1 py-0.5">
                <span className="text-[8px] uppercase font-bold text-[#4A4A5A] mr-1">Disc</span>
                <input
                  type="number"
                  ...
                  placeholder="0"
                />
                <button onClick={() => setGlobalDiscount(globalDiscount, globalDiscountType === "percentage" ? "fixed" : "percentage")} className="text-[8px] font-bold text-[#F5C842] ml-0.5">
                  {globalDiscountType === "percentage" ? "%" : curr}
                </button>
              </div>
              <div className="flex-1 flex items-center bg-[#141418] rounded-sm border border-[#1E1E26] px-1 py-0.5">
                <span className="text-[8px] uppercase font-bold text-[#4A4A5A] mr-1">Tip</span>
                <input
                  type="number"
                  value={tipAmount || ""}
              ...
              <div className="flex-[2] flex items-center bg-[#141418] rounded-sm border border-[#1E1E26] px-1 py-0.5">
                ...
                />
                <button onClick={appliedCoupon ? handleRemoveCoupon : handleApplyCoupon} className="text-[8px] font-bold text-[#F5C842] ml-1">
            ...
            <div className="flex items-center justify-between py-1.5 border-y border-white/5 text-[10px] font-bold tracking-tight">
              <div className="flex gap-2">
                <div className="flex gap-0.5"><span className="text-[#4A4A5A]">SUB:</span><span>{curr}{totals.subtotal.toFixed(2)}</span></div>
                <div className="flex gap-0.5"><span className="text-[#4A4A5A]">TAX:</span><span>{curr}{totals.tax_amount.toFixed(2)}</span></div>
              </div>
              <div className="flex gap-1 items-baseline">
                <span className="text-[#4A4A5A] text-[8px] uppercase">Total:</span>
                <span className="text-[15px] text-[#F5C842]">{curr}{finalTotal.toFixed(2)}</span>
              </div>
            </div>
            ...
            {walletCustomerId && walletBalance > 0 && (
              <div className="flex items-center justify-between bg-blue-500/5 border border-blue-500/10 rounded p-1.5">
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] text-[#9090A8]">Wallet: {curr}{walletBalance.toFixed(2)}</span>
                </div>
                <button
                  onClick={() => setUseWallet(!useWallet)}
                  className={`w-8 h-4 rounded-full transition-colors relative ${useWallet ? 'bg-[#3498DB]' : 'bg-[#1E1E26]'}`}
                >
                  <div className={`absolute top-0.5 w-3 h-3 bg-white rounded-full transition-all ${useWallet ? 'left-4.5' : 'left-0.5'}`} style={{ left: useWallet ? '18px' : '2px' }} />
                </button>
              </div>
            )}
            ...
            <div className="grid grid-cols-4 gap-1" role="group" aria-label="Payment method">
              {(["cash", "card", "upi", "split"] as const).map((m) => (
                <button
                  key={m}
                  onClick={() => { setPaymentMethod(m); if (m === "split") setShowSplitPaymentModal(true); }}
            ...
            {paymentMethod === "cash" && (
              <div className="flex flex-col gap-1">
                <div className="flex gap-2 items-center bg-[#141418] rounded border border-[#1E1E26] px-2 py-1">
                  <span className="text-[9px] uppercase font-bold text-[#4A4A5A]">Tendered</span>
                  <input
                    type="number"
                    value={amountPaid || ""}
                ...
                {amountPaid > finalTotal && (
                  <div className="flex justify-end">
                    <span className="text-[10px] font-bold text-[#2ECC71]">Change: {curr}{change.toFixed(2)}</span>
                  </div>
                )}
              </div>
            )}
            ...
            {paymentMethod === "upi" && settings.upi_id && (
              <div className="flex flex-col items-center gap-2 p-2 bg-[#141418] rounded border border-[#1E1E26]">
                <QRCodeSVG value={`upi://pay?pa=${settings.upi_id}&pn=${encodeURIComponent(settings.store_name)}&am=${finalTotal}&cu=INR`} size={80} />
            ...
            <div className="flex gap-2 pt-2 border-t border-white/5">
              ...
              <button
                ...
                disabled={processing || cart.length === 0 || (orderType === "dine_in" && !tableId)}
                ...
                data-checkout-button
              >
                {processing ? <RefreshCw size={12} className="spin" /> : <ChevronRight size={12} />}
