...
test.describe("End-to-End Flow Verification", () => {
  test("Complete Registration, Login and POS Transaction", async ({ page }) => {
    ...
    await page.screenshot({
      path: "verification/screenshots/flow_1_pin_view.png",
    });
    ...
    const onboardingVisible = await page
      .isVisible("text=Welcome to Appixen POS Billing", { timeout: 10000 })
      .catch(() => false);
    ...
    await page.waitForSelector('button[role="menuitem"]:has-text("POS")', {
      timeout: 20000,
    });
    await page.screenshot({
      path: "verification/screenshots/flow_2_pos_loaded.png",
    });
    ...
    const coffeeBtn = page
      .locator('button[role="gridcell"]:has-text("Coffee")')
      .first();
    ...
    await page.screenshot({
      path: "verification/screenshots/flow_3_order_complete.png",
    });
    ...
    // Edit Product
    await page.click(
      `tr:has-text("${productName}") button[aria-label="Edit product"]`,
    );
    await page.fill('input[name="price"]', "149.99");
    await page.click('button:has-text("Save")');
    await expect(
      page.locator(`tr:has-text("${productName}"):has-text("149.99")`),
    ).toBeVisible();

    // Delete Product
    await page.click(
      `tr:has-text("${productName}") button[aria-label="Delete product"]`,
    );
