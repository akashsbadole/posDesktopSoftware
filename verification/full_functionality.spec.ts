...
test.describe("POS System Full Functionality", () => {
  ...
  test("Authentication and Lockout Logic", async ({ page }) => {
    ...
    await expect(
      page.locator("text=Invalid PIN. 2 attempts remaining."),
    ).toBeVisible();

    await page.fill('input[type="password"]', "9998");
    await page.click('button:has-text("Login")');
    await expect(
      page.locator("text=Invalid PIN. 1 attempts remaining."),
    ).toBeVisible();
  ...
  test("Lock/unlock should accept first valid PIN attempt after lock", async ({
    page,
  }) => {
    await page.fill('input[type="password"]', ADMIN_PIN);
    await page.click('button:has-text("Login")');
    await expect(
      page.locator('div[role="status"]').filter({ hasText: "admin" }),
    ).toBeVisible();
    ...
    // First unlock attempt with correct PIN should succeed immediately
    await page.fill('input[type="password"]', ADMIN_PIN);
    await page.click('button:has-text("Login")');
    await expect(
      page.locator('div[role="status"]').filter({ hasText: "admin" }),
  ...
  test("Admin Full Workflow: Products, CRM, POS, Orders", async ({ page }) => {
    ...
    await page.selectOption(
      'select:near(label:has-text("Price Tier"))',
      "wholesale",
    ); // Wholesale gives discount
    ...
    const totalText = await page
      .locator('[aria-label^="Total amount"]')
      .innerText();
    const totalVal = parseFloat(totalText.replace(/[^0-9.]/g, ""));
    await page
      .locator('div:has(> span:has-text("card")) input')
      .fill((totalVal - 50).toFixed(2));
    ...
    await page.evaluate(() => {
      const btn = Array.from(document.querySelectorAll("button")).find(
        (b) =>
          ...
          b.getAttribute("aria-label") === "Logout",
      );
      if (btn) btn.click();
    });
  ...
  test("Cashier Access Restrictions", async ({ page }) => {
    ...
    await page.click('button:has-text("Login")');
    await expect(
      page.locator('div[role="status"]').filter({ hasText: "cashier" }),
    ).toBeVisible();

    // Try to access Settings (should be hidden or restricted)
    const settingsItem = page.locator(
      'button[role="menuitem"]:has-text("Settings")',
    );
